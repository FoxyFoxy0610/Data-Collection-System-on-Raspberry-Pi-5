...
# ---- ROI 限定 (上下拼接，取上下接縫 200 像素) ----
overlap = 1000
gray_top_crop = gray_top[-overlap:, :]
gray_bottom_crop = gray_bottom[:overlap, :]

# 建立 SIFT 偵測器
sift = cv2.SIFT_create( nfeatures=0, # 不限制特徵點數量 (0) 
                        nOctaveLayers=3, # 每層金字塔的層數 (3) 
                        contrastThreshold=0.04, # 低對比度特徵點過濾門檻 (0.04) 
                        edgeThreshold=10, # 過濾邊緣響應 (10) 
                        sigma=1.6 # 初始高斯模糊 sigma (1.6) 
                        )

# 偵測特徵點與描述子（保留原始座標）
kp1_crop, des1 = sift.detectAndCompute(gray_top_crop, None)
kp2_crop, des2 = sift.detectAndCompute(gray_bottom_crop, None)
...
if len(good) > 4:
    ...
    # 儲存結果
    cv2.imwrite("stitched_image(SIFT).jpeg", final)
    print("改進匹配策略完成，耗時:", time.time() - start_time, "秒")
else:
    print("匹配點不足，無法縫合")